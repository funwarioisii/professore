...
- TTSはmacOS標準音声とMac版AquesTalkPlayerを選択できます。AquesTalkPlayer 1.0.1の「れいむ」（AquesTalk1 f1・棒読み）で紹介動画を実生成しました。1080p/30fps、98.83秒、H.264/AACを確認し、ブラウザ再生と30／60／90秒へのシークが成功しています。[動画](../examples/professore-intro/video.mp4)・[検証記録](../examples/professore-intro/verification.json)。人による主観的な聴取品質は未検証です。コマンド引数・一時ファイル削除・失敗／キャンセル処理は代替実行ファイルで検証しました。型検査、11件のテスト、ビルドが成功しています。`mise exec -- bun scripts/tts-ui-e2e.ts` で、独立した保存先のUIから選択・プリセット保存／再読込・キャッシュ更新・未インストールエラー・標準音声への切り替えを確認しました。クラウドTTS、別OSでの実音声生成、単語単位アライメントは未対応です。
- 字幕はbeat単位の焼き込み。表示／非表示の短いフェード以外の任意アニメーション、動画タイムラインのドラッグ編集、ネイティブ包装は未対応です。
- 原稿JSONの手動編集が中心です。UIはbeatの文章・読みの上書き・字幕・TTS設定を直接編集し、図・テーマ・辞書などはJSONで編集します。画面プレビューはレイアウト確認用で、実測音声の同期は生成後の動画で確認します。
...
- Codex CLI 0.146.0のユーザーMCP登録、別ディレクトリを指定した `skills/list` でのユーザーSkill検出、登録設定を使った11ツール取得と `list_projects` を2026-09-09に確認しました。Claude Code 2.1.265の登録コマンドはCLIヘルプと公式仕様で確認しています。各製品の設定UIとQuickTimeは未検証です。GitHub Actionsのリモート実行結果はローカル検証とは別です。
