...
- TTSはmacOS標準音声とMac版AquesTalkPlayerを選択できます。AquesTalkPlayerの実機音声は未検証です（未インストール、公式配布のブラウザ認証により自動取得できず）。コマンド引数・一時ファイル削除・失敗／キャンセル処理は代替実行ファイルで検証しました。型検査、11件のテスト、ビルドが成功しています。`mise exec -- bun scripts/tts-ui-e2e.ts` で、独立した保存先のUIから選択・プリセット保存／再読込・キャッシュ更新・未インストールエラー・標準音声への切り替えを確認しました。クラウドTTS、別OSでの実音声生成、単語単位アライメントは未対応です。
